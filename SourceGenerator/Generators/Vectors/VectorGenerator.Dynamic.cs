...
public partial class VectorGenerator
{
    ...
    /// a vector that keeps its value in one and the vector itself when it has no register. Every vector of a
    /// number type implements the interface, a mask does not: the constraint of a member of a visitor names the
    /// vector interface of a number and a mask implements the one of a bool instead.
    /// </summary>
