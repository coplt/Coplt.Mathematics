...
/// <summary>
/// The rounding up of a value is the member of the algebra of the floating point kind: the value of a vector
...
public class TestCeil
{
    ...
    {
        var r = math.ceil(v);
        Assert.That(v.ceil(), Is.EqualTo(r));
    }
    ...
    public void Value()
    {
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(math.ceil(new float2(1.2f, -1.2f)), Is.EqualTo(new float2(2f, -1f)));
            Assert.That(math.ceil(new float3(1.2f, -1.2f, 2.5f)), Is.EqualTo(new float3(2f, -1f, 3f)));
            Assert.That(math.ceil(new float4(1.2f, -1.2f, 2.5f, -2.5f)), Is.EqualTo(new float4(2f, -1f, 3f, -2f)));

            // the register of a double is 128 bits wide for two components and 256 bits wide for three and four
            Assert.That(math.ceil(new double2(1.2, -1.2)), Is.EqualTo(new double2(2d, -1d)));
            ...
            Assert.That(math.ceil(new float3(0.5f, -0.5f, 0f)), Is.EqualTo(new float3(1f, 0f, 0f)));
    ...
    public void PaddingLanes()
    {
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(math.ceil(new float2(1.2f, -1.2f)).vector.GetElement(2), Is.EqualTo(0f));
            Assert.That(math.ceil(new float2(1.2f, -1.2f)).vector.GetElement(3), Is.EqualTo(0f));
            Assert.That(math.ceil(new float3(1.2f, -1.2f, 2.5f)).vector.GetElement(3), Is.EqualTo(0f));
            Assert.That(math.ceil(new double3(1.2, -1.2, 2.5)).vector.GetElement(3), Is.EqualTo(0d));
        }
    }
}
