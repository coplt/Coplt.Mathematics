...
public static partial class math
{
    ...
    {
        if (typeof(T) == typeof(float)) return Unsafe.BitCast<float, T>(F_MinNormal);
        if (typeof(T) == typeof(double)) return Unsafe.BitCast<double, T>(D_MinNormal);
        if (typeof(T) == typeof(half)) return Unsafe.BitCast<short, T>(0x0400);
    ...
    /// <para>This form reaches the values of every type, the select of a scalar type is also generated into the
    /// file of that type and the one of a vector is a member of the vector itself.</para>
    /// </summary>
    ...
    //
    // [MethodImpl(256 | 512)]
    // public static float3 slerp_unit(float3 a, float3 b, float t)
    // {
    //     var d = a.dot(b);
    //     if (1f - MathF.Abs(d) <= 1e-6f) // smaller epsilon
    //         return d > 0f
    //             ? a * (1f - t) + b * t
    ...
    //     {
    //         // (0,0,1) x (x,y,z)
    //         var im = normalize ? 1 / Math.Sqrt(sqr) : 1;
    //         // new(-v.y * im, v.x * im, 0f)
    ...
    // {
    //     var d = a.dot(b);
    //     if (1 - Math.Abs(d) <= 1e-6) // smaller epsilon
    //         return d > 0
    //             ? a * (1 - t) + b * t
